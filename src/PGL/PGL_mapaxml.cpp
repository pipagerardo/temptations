...
void MapaXML::procesaBASE64( Matriz< Uint16 > *array ) {
	...
	if( bufplain == NULL ) {
		// SDL_LOG( "Fallo de memoria" );
		return;
	}
	int retorno = Base64decode( bufplain, &m_cadena[primer] );
	if( retorno < 4 ) {
		// SDL_LOG( "Fallo decodificando Base64" );
		SDL_free( bufplain );
		return;
	}
...
void MapaXML::procesaBASE64ZLIB( Matriz< Uint16 > *array ) {
	...
	SDL_free( bufplain );
	if( retorno < 0 ) {
		// SDL_LOG( "Fallo descomprimiendo zlib" );
		contenedor.quita();
		return;
	}
...
void MapaXML::procesaBASE64GZIP( Matriz< Uint16 > *array ) {
	...
	SDL_free( bufplain );
	if( retorno < 0 ) {
		// SDL_LOG( "Fallo descomprimiendo gzip %d", retorno );
		contenedor.quita();
		return;
	}
...
void MapaXML::procesaTILES( Matriz< Uint16 > *array ) {
	...
	while( m_archivo.lee( m_cadena ) ) {
		...
		if( m_cadena.busca( "gid=\"",  m_posicion ) ) {
			...
			while( *p != '"' ) ++p; *p = '\0';
			dato = SDL_atoi( &m_cadena[ m_posicion + 5 ] );
			array->toma( x, y, (Uint16)(dato) );
			++x; 
